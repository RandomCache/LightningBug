...
namespace Examples.C2D
{
    ...
    public class MainGame : Game
    {
        ...
        Rectangle[] _rectangles = new Rectangle[3];
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            foreach (var gesture in InputHelper.Gestures)
            {
                if (gesture.GestureType == GestureType.Pinch)
                {
                    ...
                    float scale = ((distOld - dist) * 0.05f) / 10f;
                    _camera.Zoom -= scale;
                }
            ...
            if (_input.IsMousePressed(MouseButton.Left))
            {
                ...
                var coords = _camera.ToVirtual(_input.MousePos, true);
        ...
        /// </summary>
        private Point circleMovement(float center_x, float center_y, float radius)
        {
            _cmstep++;
            if (_cmstep > 360) _cmstep = 0;
            return new Point((int)(radius * Math.Cos((double)(_cmstep * 3.1415926f / 180.0f)) + center_x),
                (int)(radius * Math.Cos((double)(_cmstep * 3.1415926f / 180.0f)) + center_y));

...
namespace Examples.UIDesigner
{
    ...
    public class UIDesignerComponent : DrawableGameComponent
    {
        ...
        /// </summary>
        private Keys[] _prevKeys = new Keys[] { };
        private MouseState _prevMouseState;
        private bool _isDragging;
        private bool _isResizing;
        private bool _showTutorialInfo;
#if WINDOWS
        private UIDesigner.UIDesigner wfDesigner;
#endif
        /// <summary>
            ...
            : base(game)
        {
            ...
            ShiftSize = 3;
#if WINDOWS
            wfDesigner = new UIDesigner.UIDesigner(Game.Content, list);
#endif
            LoadContent();
        }
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                wfDesigner.UpdateData(Textures);
                if (wfDesigner.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    Textures = wfDesigner.ResultList;
                _textureIndex = 0;
                if (Textures.Count > 0)
                {
                    SelectedTexture = Textures[_textureIndex].Item1;
            ...
            if (_isResizing)
            {
                ...
                if (shiftX != 0)
                {
                    ...
                    {
                        var t_width = _tmpWidth - shiftX;
                        if (t_width >= .001)
                        {
                            // Figure out the ratio
                            float ratioX = t_width / (float)_tmpWidth;

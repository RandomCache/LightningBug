...
namespace Examples.UIDesigner.UIDesigner
{
    public partial class UIDesigner : Form
    {
        ...
        void but_copytex_Click(object sender, EventArgs e)
        {
            ...
            {
                var data = (item as DataGridViewRow).DataBoundItem as UData;
        ...
        {
            var dlg = new OpenFileDialog() { CheckFileExists = true, Filter="Supported Images|*.png;*.jpg;*.bmp", Multiselect = true, RestoreDirectory = true, Title = "Selec images to add as textures" };
            dlg.ShowDialog();
            foreach (var item in dlg.FileNames)
            {
                var tex = _content.Load<Texture2D>(item);
        ...
        class UData : IDisposable
        {
            ...
            public UData Clone()
            {
                return (UData)this.MemberwiseClone();
            }
